#ifndef TGS_TO_GIF_FSTREAM_H
#define TGS_TO_GIF_FSTREAM_H
...
// GNU-specific strerror_r()
		...
		std::string tmp(p, std::strlen((const char*) p));
		std::swap(buff, tmp);
#endif
		...
		struct static_method_holder {
			...
			static void check_mode(const std::string& filename, std::ios_base::openmode mode) {
				if ((mode & std::ios_base::trunc) && !(mode & std::ios_base::out)) {
					throw Exception(
						std::string("strict_fstream: open('") + filename + "'): mode error: trunc and not out");
				} else if ((mode & std::ios_base::app) && !(mode & std::ios_base::out)) {
					throw Exception(
						std::string("strict_fstream: open('") + filename + "'): mode error: app and not out");
